...
When the null hypothesis is rejected, the sample average of $\hat{\mu}$ is usually close to the true value of $\mu$ because these tests have enough power to spot the real effect. But when the null isn’t rejected, the average can be off due to variability or random sampling quirks. For instance, when $\mu= 1$, some sample estimates can be above 2 and still not lead to rejection if there’s a lot of variability. This shows that not rejecting the null doesn’t always mean the sample mean is close to 0—it could just mean the data had more variability or some outliers.
